...
name: phoenix
description: |
  Phoenix is an Elixir web framework built on the BEAM VM for fault-tolerant, real-time
...
Phoenix leverages the Erlang VM (BEAM) for massive concurrency and fault tolerance. LiveView enables rich, real-time UIs with server-rendered HTML.
...
defmodule MyApp.Articles do
  ...
  def create_article(attrs) do
    %Article{} |> Article.changeset(attrs) |> Repo.insert()
  end
end
```
...
defmodule MyAppWeb.ArticleLive.Index do
  ...
  def mount(_params, _session, socket) do
    {:ok, assign(socket, articles: Articles.list_published(), search: "")}
  end

  @impl true
  def handle_event("search", %{"query" => query}, socket) do
    {:noreply, assign(socket, articles: Articles.search(query), search: query)}
  end
end
```
...
defmodule MyAppWeb.RoomChannel do
  ...
  def join("room:" <> _room_id, _params, socket), do: {:ok, socket}
