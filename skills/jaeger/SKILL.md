...
Set up Jaeger for distributed tracing to track requests across microservices. Covers deployment, OpenTelemetry instrumentation, storage configuration, sampling strategies, and trace analysis.
...
```json
{
  ...
  "default_strategy": {
    ...
    "operation_strategies": [
      { "operation": "/health", "type": "probabilistic", "param": 0.001 }
    ]
  }
}
```
...
curl -s "http://localhost:16686/api/traces?service=order-service&limit=20&minDuration=500ms&lookback=1h" | \
  jq '.data[] | {traceID: .traceID, spans: (.spans | length), duration: (.spans[0].duration / 1000)}'
```
...
- Sample 100% of traces for critical paths like payments
- Add meaningful span attributes (order IDs, user IDs, status codes) for debugging
- Use Elasticsearch or Cassandra for production; badger is for development only
