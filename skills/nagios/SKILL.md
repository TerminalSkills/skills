...
Set up Nagios Core for infrastructure and service monitoring with host definitions, check commands, notification contacts, and custom plugins.

## Instructions

### Task A: Install Nagios
...
```cfg
# /etc/nagios4/servers/web-servers.cfg — Host definitions
define host {
    use                     linux-server
    host_name               web-01
    alias                   Web Server 01
...
### Task C: NRPE Configuration

```cfg
# /etc/nagios4/conf.d/commands.cfg — Check command definitions
define command {
...
```cfg
# /etc/nagios/nrpe.cfg — NRPE on remote hosts
...
```cfg
# /etc/nagios4/conf.d/contacts.cfg — Contact definitions
define contact {
    contact_name                    marta
...
```bash
#!/bin/bash
# /usr/lib/nagios/plugins/check_api_health — Custom API health check
URL=""
WARN=1000
CRIT=3000
...
if [ "$RESPONSE" != "200" ]; then
    echo "CRITICAL - HTTP $RESPONSE from $URL | response_time=${DURATION}ms"; exit 2
elif [ "$DURATION" -gt "$CRIT" ]; then
    echo "CRITICAL - ${DURATION}ms > ${CRIT}ms | response_time=${DURATION}ms"; exit 2
elif [ "$DURATION" -gt "$WARN" ]; then
    echo "WARNING - ${DURATION}ms > ${WARN}ms | response_time=${DURATION}ms"; exit 1
else
    echo "OK - ${DURATION}ms | response_time=${DURATION}ms"; exit 0
fi
```
...
- Always run `nagios -v` to verify config before reloading
...
- Use hostgroups to apply checks to multiple hosts at once
- Output performance data (`| metric=value`) from plugins for graphing
